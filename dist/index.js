...
client.on(discord_js_1.Events.VoiceStateUpdate, async (oldState, newState) => {
    try {
        ...
        if (oldState.channelId && !newState.channelId) {
            ...
            try {
                const entry = await fetchAuditLog(guild, discord_js_1.AuditLogEvent.MemberDisconnect, undefined // MemberDisconnect nem sempre tem targetId confiável
                );
    ...
    try {
        ...
        if (message.attachments && message.attachments.size > 0) {
            const attachmentList = message.attachments
                .map((att) => `[${att.name || "arquivo"}](${att.proxyURL || att.url}) (${att.contentType || "desconhecido"})`)
                .join("\n");
            embed.addFields({
                name: "📎 Anexos",
